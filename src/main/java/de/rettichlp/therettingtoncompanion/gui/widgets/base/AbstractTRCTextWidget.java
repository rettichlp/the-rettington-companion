...
public abstract class AbstractTRCTextWidget<C extends WidgetConfiguration> extends AbstractTRCWidget<C> {
    ...
    public void extractWidget(@NotNull GuiGraphicsExtractor graphics, int x, int y, Color color, boolean backgroundEnabled, boolean textShadowEnabled) {
        ...
        graphics.text(this.minecraft.font, text(), x + getWidth() / 2 - this.minecraft.font.width(text()) / 2, y + getHeight() / 2 - this.minecraft.font.lineHeight / 2, color.getRGB(), textShadowEnabled);
    }
}
